		...
		case "JOIN":
			if (player.get("autoPause") === true) {
					...
					let playerResumed = new MessageEmbed()
						...
						.setAuthor({ name: `Đã tiếp tục !`, iconURL: client.config.iconURL})
						.setDescription(
			...
			if (player.get("autoPause") === true) {
			    ...
			    if (members === 0 && !player.paused && player.playing){
					...
					let playerPaused = new MessageEmbed()
						.setColor(client.config.embedColor)
						.setTitle(`Đã tạm dừng !`, client.config.iconURLStopped)
						.setDescription(
							`<:shell32:1044949839147974696> Bot phát hiện không có ai trong voice nên đã dừng nhạc !`,
							);
